                ...
                <section className="mb-8">
                    ...
                    <div className="grid gap-6">
                        {recruitmentData.map((job, index) => (
                            ...
                            <div key={index} className="bg-gray-50 p-5 shadow-md rounded-lg border-l-4 border-primary">
                                <h3 className="text-xl font-semibold text-gray-900">{job.title}</h3>
                                <p className="text-sm text-gray-600 mt-1">📅 Date: {job.date} | ⏳ Last Date: {job.lastdate}</p>
                                <p className="text-gray-700 mt-2">{job.description}</p>
                                <a href={job.document} target="_blank" rel="noopener noreferrer" className="mt-3 inline-block text-primary font-semibold hover:underline">
                                    📄 Download Document
                                </a>
                            </div>
                        ))}
