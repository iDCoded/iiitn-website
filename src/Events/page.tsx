...
const Events = () => {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        const res = await axios.get("http://localhost:5000/card/cards/categories/events");
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-12">
      <h1 className="text-3xl font-bold text-primary mb-6 text-center">Upcoming Events</h1>
        ...
        {subCategories.map((subCategory) => (
          <button
            ...
            className={`relative pb-2 text-lg font-semibold transition-all 
                            ${activeTab === subCategory ? "text-accent" : "text-gray-500"}
                        `}
          >
            {subCategory}
