    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Separate faculty and staff correctly
                const facultyList = response.data.filter((item: Faculty) => item.department === id && item.f_or_s === "faculty");
                const staffList = response.data.filter((item: Staff) => item.department === id && item.f_or_s === "staff");

                if (response.data.length > 0) {
                    setFacultyData(facultyList);
                    setStaffData(staffList);
                } else {
                    let filterfacultyDemoData = facultyDemoData.filter((item: Faculty) => item.department === id);
                    let filterstaffDemoData = staffDemoData.filter((item: Staff) => item.department === id);
                    setFacultyData(filterfacultyDemoData);
                    setStaffData(filterstaffDemoData);
                }
            } catch (error) {
