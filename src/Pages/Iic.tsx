		...
		const fetchData = async () => {
			try {
				const res = await fetch(
					`${import.meta.env.VITE_API_BASE_URL}/card/cards/category/iic`
				);
				...
				const about = data.find(
					(item: IIICData) => item.title.toLowerCase() === "about"
				);
				const faculty = data.find(
					(item: IIICData) => item.title.toLowerCase() === "faculty"
				);
				const achievements = data.find(
					(item: IIICData) => item.title.toLowerCase() === "achievements"
				);

				setAboutData(about || { title: "About", content: "No data available" });
				setFacultyData(
					faculty || { title: "Faculty", content: "No data available" }
				);
				setAchievementData(
					achievements || {
						title: "Achievements",
		...
		<div className="bg-gray-100 min-h-screen flex flex-col">
			...
			<div className="max-w-6xl mx-auto p-6 space-y-8">
				...
				<section className="bg-white shadow-lg rounded-lg p-6">
					...
					<MarkdownPreview
						source={
							achievementData ? achievementData.content : "No data available"
						}
						wrapperElement={{ "data-color-mode": "light" }}
						className="prose max-w-none !bg-white !text-gray-800 p-4 rounded-md shadow"
					/>
