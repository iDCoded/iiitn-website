...
const AlumniSection = () => {
    ...
    return (
        <div className="relative flex flex-col items-center justify-center min-h-screen w-full bg-gradient-to-br from-[#002147] via-[#19376D] to-[#070F2B] text-white px-4 sm:px-6 lg:px-12 py-16 sm:py-20">
            {/* Background Image */}
            ...
            {/* Heading Section */}
            <div className="relative z-10 flex flex-col items-center justify-center text-center py-10 sm:py-12">
                <motion.h2
                ...
                {/* Description */}
                <motion.p
                    className="max-w-4xl mx-auto text-center px-4 sm:px-8 mt-3 text-gray-200 text-base sm:text-lg"
                    initial={{ opacity: 0, y: 30 }}
                    animate={inView ? { opacity: 1, y: 0 } : {}}
            ...
            <div ref={ref} className="relative z-10 w-full max-w-3xl p-6 sm:p-10 bg-white/20 backdrop-blur-lg shadow-lg border border-white/30 rounded-lg">
                <AnimatePresence mode="wait">
                    ...
                    >
                        <div className="w-24 h-24 sm:w-32 sm:h-32 rounded-full bg-cover bg-center border-4 border-white/50 shadow-lg" style={{ backgroundImage: `url(${alumniData[index].image})` }}></div>

                        <h2 className="text-xl sm:text-2xl font-bold text-white drop-shadow-md">{alumniData[index].name}</h2>
                        <p className="text-orange-400 text-md sm:text-lg font-semibold italic">{alumniData[index].position}</p>

                        <p className="mt-4 text-base sm:text-lg font-medium text-white/80 px-4 sm:px-8 leading-relaxed">
                ...
                <div className="flex justify-center mt-6 space-x-2 sm:space-x-3">
                    {alumniData.map((_, i) => (
                        <button
                            key={i}
                            onClick={() => setIndex(i)}
                            className={`w-3 h-3 sm:w-4 sm:h-4 rounded-full transition-all duration-300 ${i === index ? 'bg-white w-4 h-4 sm:w-5 sm:h-5' : 'bg-gray-400 opacity-70'}`}
                        ></button>
                    ))}
                </div>
            </div>
