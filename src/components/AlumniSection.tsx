...
const alumniData = [
    ...
    { name: "Rutvik Page", position: "Research Professional, University of Chicago", image: AD, quote: "The perenially available and efficient Training and Placement Cell (TnP) does an impeccable task of exposing the students to world-class facilities like webinars and fantastic job opportunities." },
        ...
        <div className="relative flex flex-col items-center justify-center min-h-screen w-full bg-gradient-to-br from-[#002147] via-[#19376D] to-[#070F2B] text-white">
            ...
            <div className="relative w-full max-w-2xl p-6 rounded-xl bg-white/20 backdrop-blur-lg shadow-lg border border-white/30">
                <AnimatePresence mode="wait">
                    ...
                    >
                        <div className="w-32 h-32 rounded-full bg-cover bg-center border-4 border-white/50 shadow-lg" style={{ backgroundImage: `url(${alumniData[index].image})` }}></div>
                        <h2 className="mt-4 text-2xl font-bold">{alumniData[index].name}</h2>
                        <p className="mt-1 text-orange-400 text-lg font-semibold italic">{alumniData[index].position}</p>
                        <p className="mt-4 text-lg font-medium text-white/80 px-6 leading-relaxed">"{alumniData[index].quote}"</p>
                    </motion.div>
                </AnimatePresence>
                ...
                <div className="flex justify-center mt-4 space-x-2">
                    {alumniData.map((_, i) => (
                        <button 
                            key={i} 
                            onClick={() => setIndex(i)} 
                            className={`w-3 h-3 rounded-full transition-all duration-300 ${i === index ? 'bg-white w-4 h-4' : 'bg-gray-400'}`}>
                        </button>
                    ))}
                </div>
            </div>
